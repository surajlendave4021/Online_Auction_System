...
from bid_app.send_email import sendemail
from .models import *
...
def home(request):
    ...
    if request.user.is_authenticated:
        ...
        auctionuser = AuctionUser.objects.filter(user=request.user).first()

        if auctionuser and auctionuser.status == "pending":
            messages.success(
                request,
                "Your verification is pending. complete your additional detail and email verification."
            )
...
def admin_home(request):
    ...
    pending_bidder = AuctionUser.objects.filter(
        user_type="Bidder",
        status="pending"
    )

    approved_bidder = AuctionUser.objects.filter(
        user_type="Bidder",
        status="Approved"
    )
    ...
    pending_seller = AuctionUser.objects.filter(
        user_type="Seller",
        status="pending"
    )

    approved_seller = AuctionUser.objects.filter(
        user_type="Seller",
        status="Approved"
    )
    ...
    pending_auction = Product.objects.filter(
        bid_type="Auction",
        status="pending"
    )

    live_auction = Product.objects.filter(
        bid_type="Auction",
        status="live"
    )

    closed_auction = Product.objects.filter(
        bid_type="Auction",
        status="closed"
    )
    ...
    pending_tender = Product.objects.filter(
        bid_type="Tendor",
        status="pending"
    )

    live_tender = Product.objects.filter(
        bid_type="Tendor",
        status="live"
    )

    closed_tender = Product.objects.filter(
        bid_type="Tendor",
        status="closed"
    )
    ...
    d = {
        ...
        # Bidder
        'bidder': bidder.count(),
        'pending_bidder': pending_bidder.count(),
        'approved_bidder': approved_bidder.count(),

        # Seller
        'seller': seller.count(),
        'pending_seller': pending_seller.count(),
        'approved_seller': approved_seller.count(),

        # Auction
        'auction': auction.count(),
        'pending_auction': pending_auction.count(),
        'live_auction': live_auction.count(),
        'closed_auction': closed_auction.count(),

        # Tender
        'tender': tender.count(),
        'pending_tender': pending_tender.count(),
        'live_tender': live_tender.count(),
        'closed_tender': closed_tender.count(),
    }
...
def generateotp(request, pid):
    try:
        ...
        checkemail = sendemail(user.user.email, user.user.username, otp)
        
        if checkemail:
            # Save the OTP if the email was sent successfully
            user.otp = otp
            user.save()
